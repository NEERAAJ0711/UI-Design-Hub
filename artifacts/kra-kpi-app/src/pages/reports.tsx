...
import {
  ...
  FileText, Download, LayoutDashboard, Loader2,
} from "lucide-react";
...
export default function Reports() {
  ...
  function handleDownload(id: string) {
    ...
    setTimeout(() => {
      try {
        if (id === "overview") {
          generateCompanyOverviewPDF(summary ?? {}, deptPerf ?? [], topPerformers ?? []);
        } else if (id === "department") {
          generateDepartmentReportPDF(deptPerf ?? []);
        } else if (id === "kpi") {
          generateKpiReportPDF(kpis ?? []);
        } else if (id === "tasks") {
          generateTaskReportPDF(tasks ?? []);
        }
      } finally {
        setGenerating(null);
      }
